...
def run_stress(tri: Triage, runs: int) -> dict:
    pairs = [(r, _mk(r)) for r in json.loads((EVAL_DIR / "stress_set.json").read_text(encoding="utf-8"))]
    ...
    for row, t in pairs:
        ...
        # priority sanity
        if cat in ("paraphrase", "german", "french", "misleading_title") and not g["unclear"]:
            sane_n += 1
...
def main(offline: bool = False, runs: int = 5) -> int:
    ...
    holdout = run_holdout(Triage(retriever=ret, use_llm=False if offline else None))
    base = baselines(load_training())
    s = get_settings()
    res = {"created": time.strftime("%Y-%m-%dT%H:%M:%S"), "llm_enabled": tri.llm_on, "provider": s.llm_provider, "model": s.llm_model if tri.llm_on else None,
           "embedder": ret.embedder.name, "stress": stress, "holdout": holdout, "baselines": base}
    (EVAL_DIR / "results.json").write_text(json.dumps(res, indent=2, ensure_ascii=False), encoding="utf-8")
    (EVAL_DIR / "RESULTS.md").write_text(render_markdown(res), encoding="utf-8")

...
Team 8 — Swiss AI Weeks hackathon: AI-powered ticket triage.

## Swiss AI Weeks Hackathon Challenge: AI-Powered Ticket Triage

### Run locally

Prereqs: [Aspire CLI](https://aspire.dev/get-started/install-cli/), Node 22+, [uv](https://docs.astral.sh/uv/).

```bash
cp backend/.env.example backend/.env   # paste the shared Foundry API key (or enter it in the dashboard on first run)
npm install
aspire run
```

`apphost.mts` (TypeScript AppHost) starts the FastAPI backend (`backend/`, via uv) and the React frontend
...
#### 2. The challenge file — `jira_hackathon_blind_eval_challenge_20260923083915-1141.json`

...
|---|---|---|---|
| **00–12s** | **The Hook: Intake Prevention & Real-Time Qualification**<br>Employee opens Intake Partner. Vague report is intercepted before ticket creation; readiness gauge pulses as qualifications begin. | **Compound Dual-Engine Architecture**<br>Visual schematic lights up: Pre-LLM Intake Shield at the perimeter connecting to the scalable, deterministic core (`Main2`). | “The best ticket is the one that never gets created—and the second best arrives fully qualified. That’s exactly what we delivered with our product, team8.” |
| **12–14s** | *[Visual Breather & Transition]*<br>Screen cuts to routine employee inquiry. | *[Telemetry Pulse]*<br>Real-time stream switches from architecture view to live zero-ticket deflection monitoring. | *(0.5s pause / eye contact)* |
| **14–27s** | **Zero-Ticket Deflection: Mail Client FAQ**<br>Employee enters: *“What mail client do we use for company email?”*<br>Instant verified self-service answer surfaces: **“Microsoft Outlook (Exchange Online) via SSO”** with direct setup link.<br>Employee clicks *"That solved it!"* | **Semantic Intent Matcher & Zero-Ticket Gate**<br>Under-the-hood dataflow pulses: NLP intercepts informational intent → matches verified KB card with high confidence (>0.9) → triggers instant deflection.<br>Dashboard counter ticks: **+1 Ticket Deflected (0s MTTR)**. | “Watch: an employee asks a routine question—‘Which mail client do we use?’ Instead of clogging the queue with another ticket, Intake answers instantly from verified enterprise knowledge. Problem solved in seconds—with zero tickets created.” |
| **27–28s** | *[Visual Cut & Telemetry Transition]*<br>Employee submits an actual incident that cannot be resolved via FAQ. Cut to TriageMate Cockpit. | *[Pipeline Ingestion]*<br>Data stream flows: Intake API ➔ Pre-LLM Sanitizer ➔ TriageMate Core Engine. | *(Quick visual transition)* |
| **28–44s** | **Real Ticket Creation & Perfect Triage with Proposed Resolution**<br>Issue cannot be resolved at intake → ticket is created fully qualified.<br>Inside TriageMate Cockpit, the ticket appears perfectly triaged in real time:<br>• Priority: **P2 - High**<br>• Assigned: **ERP Core Team**<br>• Safety: PII sanitized (IBANs/names masked)<br>• Resolution: Grounded proposal pre-filled with verified citation `[KB-14]`. | **Zero-Leak Safety Gate & Deterministic ITIL 5×5**<br>Animation displays:<br>1. **Pre-LLM Privacy Shield:** IBANs and employee details scrubbed.<br>2. **Deterministic 5×5 Matrix:** Business-rule priority calculation with zero LLM drift.<br>3. Vector search retrieves matching verified enterprise playbook. | “But when an issue can’t be solved on the spot, a real ticket is born—not as vague noise, but fully qualified. Look at the result: a perfectly triaged ticket, deterministically prioritized, routed to the right team, with an actionable resolution proposal ready to go.” |
| **44–45s** | *[Visual Transition to Results]*<br>L2 Analyst approves ticket. Screen transitions to multi-model evaluation dashboard. | *[Telemetry Convergence]*<br>Data pipeline logs dispatch event and aggregates benchmark metrics. | *(1s visual beat)* |
| **45–60s** | **Multi-Model Benchmark & The Full Picture (Final Slide)**<br>1. **Multi-Model Comparison:** Radar chart and metrics comparing Swisscom Apertus and frontier models (accuracy, cost, latency).<br>2. **Final Thank You Slide:** Converges the entire journey into the full picture badge: **Intake Partner ➔ TriageMate Core ➔ Human in the Loop** (with Day 1 PoC fallback). Team 8 closing slide with QR code. | **Enterprise Compound AI Architecture Summary**<br>Final architecture animation pulses all connected systems: Requester ➔ Security Shield ➔ Deterministic ITIL Core ➔ Knowledge Playbooks ➔ ITSM.<br>Key metrics highlighted: **-40% Deflection, 0s Triage Delay, 100% ITIL Consistency, 0 PII Leaks**. | “Thanks team8, you did gr8! Our multi-model approach benchmarks Swiss models to deliver proven results: zero triage delay, forty percent deflection, and deterministic consistency. From intake prevention to human approval—that’s the full picture. Thank you!” |

---

## 2. Split-Screen Production & Delivery Guidelines

* **Target Duration:** Exactly 60 seconds (1:00 total).
* **Word Count:** 148 words total (calibrated at a natural, no-stress 148 words/min cadence).
